...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in user_access and user_access[user_id]["expires_at"] > datetime.now():
        # If the command includes a video ID, check and retrieve the video
        if args:
            video_id = args[0]
    ...
    if short_link:
        user_access[user_id] = {"number": random_number, "expires_at": datetime.now() + timedelta(hours=24)}
...
async def handle_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in user_access and user_access[user_id]["expires_at"] > datetime.now():
        ...
        # Save video file ID with the unique ID in video mapping
        video_mapping = load_video_mapping()
